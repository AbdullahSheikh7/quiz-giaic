...
type TenStringsTuple = [string, string, string, string, string, string, string, string, string, string]
...
let questions: TenStringsTuple = ["What is the capital of France?", "Who wrote 'Romeo and Juliet'?", "What is the chemical symbol for gold?", "In which year did World War II end?", "What is the largest mammal on Earth?", "Who painted the Mona Lisa?", "Which planet is known as the 'Red Planet'?", "What is the currency of Japan?", "Who is the author of 'To Kill a Mockingbird'?", "What is the main ingredient in guacamole?"]
let correctAnswers: TenStringsTuple = ["Paris", "William Shakespeare", "Au", "1945", "Blue Whale", "Leonardo da Vinci", "Mars", "Yen", "Harper Lee", "Avocado"]
...
while (true) {
  ...
  let answers: { [q: string]: string } = await inquirer.prompt([
    {
      name: "q1",
      type: "list",
      message: questions[0],
